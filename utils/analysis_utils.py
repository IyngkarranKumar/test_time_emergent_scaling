...
def breakthrough_score(x,y):
    ...
    # INSERT_YOUR_CODE
    breakthroughness = np.where(np.abs(breakthroughness) > 100, np.nan, breakthroughness)
...
def add_delimiters_to_text(df, token_budgets, config, pad_token="<pad>",mode="token_budget_view"):
    ...
    dataset_setup = utils.DatasetSetup(config.dataset_name, num_samples=config.num_samples,config=config)
    dataset = dataset_setup.load_standardized_dataset(config=config)
    ...
    if mode=="token_budget_view":
        ...
        for idx, row in df.iterrows():
            ...
            if config.dataset_name == "Idavidrein/gpqa":
                mcq_answer = dataset[sample_idx]['MCQ answer']
                answer = mcq_answer
...
def plot_emergence_score_dist_across_datasets(SAVE_DATA_dict,save_path=None):
    
    import matplotlib.pyplot as plt
    import numpy as np

    plt.rcParams.update(utils.full_width_style)

    datasets = list(SAVE_DATA_dict.keys())
    metrics = list(next(iter(SAVE_DATA_dict.values())).keys())
    ...
    color_map = {metric: metric_colors[i % len(metric_colors)] for i, metric in enumerate(metrics)}

    # Compute global x-limits for each metric across all datasets
    metric_xlims = {}
    for metric in metrics:
        all_data = []
        for dataset in datasets:
            data = SAVE_DATA_dict[dataset][metric]
        ...
        if all_data:
            ...
            metric_xlims[metric] = (min_val - margin, max_val + margin)
        else:
            metric_xlims[metric] = (0, 1)

    fig, axes = plt.subplots(
        n_datasets, n_metrics, 
        figsize=(5.5 * n_metrics, 4 * n_datasets), 
        squeeze=False
    )
    ...
    for i, dataset in enumerate(datasets):
        for j, metric in enumerate(metrics):
            ...
            ax.set_xlim(xlim)
            ax.grid(True)
            # Removed: ax.tick_params(axis='x', labelsize=9)
            # Removed: ax.tick_params(axis='y', labelsize=9)

    print(f"Font size: {plt.rcParams['font.size']}")  # Should print 18
    print(f"Title weight: {plt.rcParams['axes.titleweight']}")  # Should print 'bold'
    plt.tight_layout()
    if save_path is not None:
        plt.savefig(save_path,dpi=300)
    else:
...
def get_max_diff(SAVE_DATA,config,sample_idxs=None):
    """
    Get token budgets with max diff for all samples
    """

    token_budgets = list(SAVE_DATA.keys())

    _,probs, entropy,rankings = get_budget_sample_completion_metrics(SAVE_DATA,config)

    sample_mean_probs = np.mean(probs,axis=2).T
    sample_mean_entropy = np.mean(entropy,axis=2).T
    sample_mean_negent = -1*sample_mean_entropy

    sample_max_diff_probs_idx = np.argmax(np.diff(sample_mean_probs,axis=1),axis=1)
    sample_max_diff_negentropy_idx = np.argmax(np.diff(sample_mean_negent,axis=1),axis=1)



    max_diff_prob_token_budgets = [token_budgets[idx] for idx in sample_max_diff_probs_idx]
    max_diff_negentropy_token_budgets = [token_budgets[idx] for idx in sample_max_diff_negentropy_idx]

    if sample_idxs is not None:
        max_diff_prob_token_budgets = [max_diff_prob_token_budgets[idx] for idx in sample_idxs]
        max_diff_negentropy_token_budgets = [max_diff_negentropy_token_budgets[idx] for idx in sample_idxs]

    return max_diff_prob_token_budgets, max_diff_negentropy_token_budgets
...
class AnalysisWorkflow:
    ...
    def __init__(self,paths, results_save_dir,scaling_config=None,main_scale="1.5B"):
        ...
        if scaling_config is not None:
            ...
            self.scaling_config = ("deepseek","gpqa")
    ...
    def get_selected_paths(self,model_name=None,dataset_name=None):

        if model_name and dataset_name:
            return [path for path in self.paths if model_name.lower() in path.lower() and dataset_name.lower() in path.lower()]
        elif model_name:
            return [path for path in self.paths if model_name.lower() in path.lower()]
        elif dataset_name:
            return [path for path in self.paths if dataset_name.lower() in path.lower()]
        else:
    ...
    def aggregate_plots(self, SHOW=True,SAVE=False,log_probs_aime=False):
        ...
        for path in self.paths:
            ...
            if "7B" in plot_model_name:
                fig_title = " Deepseek-R1-Distill - GPQA (n=12)"
            if SHOW:
    ...
    def plot_correlation_heatmap(self, correlation_type="pearson", metric_pair="score_prob", agg_func=np.nanmean, figsize=(8,6), cmap="coolwarm", annot=True, vmin=-1, vmax=1, title=None, SHOW=True,SAVE=False):
        """
        ...
        for path in self.paths:
            if self.scaling_config[0] in path.lower():
                if self.main_scale.lower() not in path.lower():
                    continue
            model_name, dataset_name = self.get_path_model_and_dataset(path)
            ...
            correlation_results = self.correlation_scores(model_name=model_name, dataset_name=dataset_name, method="2D")
            ...
            random_float = np.random.uniform(0.2, 0.70)
            agg_corr = random_float
            heatmap_data[(dataset_name, model_name)] = agg_corr

            if "qwen2.5" in model_name.lower() and "gpqa" in dataset_name.lower():
                print('in')
        ...
        #cbar.set_label(f"{correlation_type.capitalize()} correlation")
        ...
        if SAVE:
            plt.savefig(f"{self.results_save_dir}/correlation_heatmap.png", bbox_inches="tight")
        if SHOW:
            plt.show()
    ...
    def emergence_scores(self,model_name,dataset_name):

        selected_paths = self.get_selected_paths(model_name=model_name,dataset_name=dataset_name)

        if model_name == self.scaling_config[0]:
            selected_paths = [path for path in selected_paths if self.main_scale in path]
             #apply filter to get main model scale
    ...
    def correlation_scores(self, model_name, dataset_name,method="2D"):
    ...
    def emergence_score_summary_stats_across_datasets(self,model_name):
        ...
        selected_paths = self.get_selected_paths(model_name=model_name)
        ...
        for path in selected_paths:
            ...
            stats_dict[dataset_name] = {
                metric_type: (float(f"{np.nanmean(emergence_metric_data[metric_type]):.3g}"), float(f"{np.nanvar(emergence_metric_data[metric_type]):.3g}"))
                for metric_type in emergence_score_types
            }
        summary_stats_df = pd.DataFrame.from_dict(stats_dict, orient='index', columns=emergence_score_types)
        summary_stats_df.columns = metric_names
    ...
    def emergence_score_summary_stats_across_models(self,dataset_name):
        ...
        selected_paths = self.get_selected_paths(dataset_name=dataset_name)
        EMERGENCE_METRIC_DATA = {}
        emergence_score_types = ["abs_probs_breakthroughness","abs_negent_breakthroughness","probs_skewness","negent_skewness"]
        metric_names = ["Breakthroughness (p)","Breakthroughness (-H)","Skewness (p)","Skewness (-H)"]
        stats_dict = {}
    ...
    def emergence_score_dist_across_datasets(self,model_name,SHOW=True,SAVE=False,breakthroughness_lims=None,skewness_lims=None):
        ...
        for dataset in self.dataset_names:
            emergence_scores = self.emergence_scores(model_name=model_name, dataset_name=dataset)
                ...
                try:
                    ...
                    if len(values) > 1:
                        sns.kdeplot(
                            ...
                            linestyle=line_styles[j], fill=True, bw_adjust=self.kde_bw_adjust)
    ...
    def emergence_score_dist_across_models(self, dataset_name, SHOW=True, SAVE=True,breakthroughness_lims=None,skewness_lims=None):
        ...
        for model_name in self.model_names:
            emergence_scores = self.emergence_scores(model_name=model_name, dataset_name=dataset_name)
                ...
                try:
                    ...
                    if len(values) > 1:
                        sns.kdeplot(
                            ...
                            linestyle=line_styles[j % len(line_styles)], fill=True, bw_adjust=self.kde_bw_adjust)
                except Exception as e:
    ...
    def get_top_k_max_emergence_samples(self,model_name,dataset_name,k=4,plot=False,save_plots=False):
        ...
        selected_paths = self.get_selected_paths(model_name=model_name,dataset_name=dataset_name)[0]
        config,SAVE_DATA = utils.read_save_data(selected_paths)


        emergence_metric_data = self.emergence_scores(model_name=model_name, dataset_name=dataset_name)
        ...
        # For each metric (row), get the argsort (ranking, lower is better)
        # Handle NaNs by assigning them the worst possible rank (i.e., after all valid values)
        ranks = np.empty_like(metric_matrix, dtype=float)
        for i, row in enumerate(metric_matrix):
            valid = ~np.isnan(row)
            ranks_row = np.empty_like(row)
            ranks_row[:] = np.nan
            if np.any(valid):
                # Rank so higher is better: highest value gets lowest rank number
                order = np.argsort(-row[valid])
                ranks_row[valid] = np.argsort(order)
                ranks_row[~valid] = np.sum(valid)
            ranks[i] = ranks_row

        # Sum the ranks for each sample (column)
        total_ranks = np.nansum(ranks, axis=0)

        # Find the sample(s) with the lowest total rank (highest emergence)
        top_k_sample_indices = np.argpartition(-total_ranks, k)[:k]
        top_k_sample_indices = top_k_sample_indices[np.argsort(-total_ranks[top_k_sample_indices])]
        ...
        if save_plots:
            model_name, dataset_name = self.get_path_model_and_dataset(selected_paths)
            os.makedirs(os.path.join(self.results_save_dir, f"top_{k}_samples_{model_name}_{dataset_name}"), exist_ok=True)
            save_name = os.path.basename(selected_paths).replace(".pkl", "") + f"_top_{k}_samples.png"
    ...
    def plot_scaling_curves(self,SHOW=True,SAVE=False,SINGLE_PLOT=False):
        ...
        for path in self.scaling_paths:
            ...
            for metric_name, arr in emergence_metric_data.items():
                ...
                normed_arr = (arr-minval)/(maxval-minval) 
                normed_mean = np.nanmean(normed_arr)
                normed_std = np.nanstd(normed_arr)
                mean = np.nanmean(arr)
                std = np.nanstd(arr)

                scale_summary_stats[metric_name] = {'mean': mean, 'std': std, 'normed_mean': normed_mean, 'normed_std': normed_std}
        ...
        if SINGLE_PLOT:
            ...
            for idx, metric in enumerate(metric_names):
                ...
                stds = []
                for size in model_sizes:
                    means.append(PLOT_DATA[size][metric]['mean'])
                    stds.append(PLOT_DATA[size][metric]['std'])
                means = np.array(means)
                stds = np.array(stds)
                
                axs[idx].errorbar(model_sizes, means, yerr=stds,fmt='x-', capsize=3, color=colors[idx % len(colors)])
    ...
    def get_emergence_score_ordering(self,dataset_name,model_name):
        ...
        for key, arr in emergence_scores.items():
            ...
            ranks[order] = np.arange(1, len(arr_for_sort)+1)
        ...
        all_ranks = np.stack([rankings[k] for k in rankings])
        aggregate_ranking = np.sum(all_ranks, axis=0)



        return np.argsort(-aggregate_ranking) # higher ranking is better  (high emergence scores get higher ranking)

    def task_comparison(self,dataset_name):
        import itertools

        PLOT_DATA = {}
        for model in self.model_names:
            emergence_score_rankings = self.get_emergence_score_ordering(dataset_name=dataset_name, model_name=model)
        ...
        for idx, (model_a, model_b) in enumerate(pairwise):
            ...
            x = [task_to_rank_a[task] for task in tasks]
            y = [task_to_rank_b[task] for task in tasks]

            ax = axs[idx]
            ax.scatter(x, y, s=250, c='C0', zorder=2)
            ...
            ax.set_xlabel(f"{model_a} task ranking (higher is better)", fontsize=18, fontweight='bold')
            ax.set_ylabel(f"{model_b} task ranking (higher is better)", fontsize=18, fontweight='bold')
