...
def batch_pad_inputs(inputs,tokenizer,device):
    ...
    for seq in inputs:
        ...
        if "scotia" in socket.gethostname().lower(): #scotia causes some problems - so go via cpu
            cpu_seq = seq.cpu()
            padded_seq_cpu = torch.concat([
                torch.full((pad_length,), tokenizer.pad_token_id, dtype=cpu_seq.dtype, device='cpu'),
                cpu_seq
            ])
            padded_seq = padded_seq_cpu.to(device)
        else:
...
def generate_with_vllm_or_hf(input_ids, attention_mask, gen_params, inference_engine, tokenizer, vllm_model, hf_model, max_new_tokens):
    ...
    if inference_engine=="vllm":
        ...
        if gen_params.get("bad_words_ids") is not None:
...
def get_vllm_model_and_tokenizer(
    ...
    if vllm_quantization is not None:
        ...
        if "damnii" in socket.gethostname().lower():
            print("DAMNII DETECTED, setting dtype to float16")
            dtype="float16"
        else:
            dtype="bfloat16"
    else:
    ...
    if "scotia" in hostname.lower(): #scotia causes some problems
        
        if vllm_quantization is not None:
            vllm_quantization = "fp8" #this seems to work rather than "bitsandbytes"
        load_format = "auto"
        dtype = "bfloat16" #all non-quantized vals
        vllm_kwargs = {
            "quantization": vllm_quantization,
            "load_format": load_format,
        } 
        ...
        if logger is not None:
            logger.info("SCOTIA DETECTED, setting custom params for vllm model loading...")
            logger.info(f"vLLM loading params:\n"
                        f"  model_path={model_path}\n"
                        f"  dtype={dtype}\n"
                        f"  tensor_parallel_size={tensor_parallelism}\n"
                        f"  gpu_memory_utilization={gpu_memory_utilization}\n"
                        f"  quantization={vllm_quantization}\n"
                        f"  enforce_eager={enforce_eager}\n"
                        f"  download_dir={os.getenv('HF_CACHE_PATH')}\n"
    ...
    vllm_model = LLM(
        ...
        **vllm_kwargs, #to deal with case with quant = None
    )
...
def check_for_force_continue_repetition(FORCE_CONTINUE_TOKEN_TRACKER,force_end_samples, n_tokens_gen_threshold=50):

    """
    Checks the text that is sandwiched between force continues for repetition
    """
    ...
    for local_idx, (global_idx, sequence_tracker_dict) in enumerate(FORCE_CONTINUE_TOKEN_TRACKER.items()): 

        if sequence_tracker_dict:  # only check if dict is not empty
            # If any value in the dict is less than or equal to threshold, mark as repetitive
            for v in sequence_tracker_dict.values():
...
def track_force_continue_generations(sequence,tokenizer,config):
    ...
    """
    Finds the number of tokens that is sandwiched between force continues
    """
    ...
    n_force_continues = len(split_sequence)-1

    force_continue_tracker = {}
    for split_idx,split in enumerate(split_sequence):
            if split_idx == 0 or split_idx == len(split_sequence)-1: #we
                continue #text in first or last split is not sandwiched between force continues
            else:
...
def batch_process_force_end_sequences(model,force_end_inputs,force_end_attention_mask,force_end_gen_params,max_new_tokens,batch_size=None):
    ...
    #catching runtime error - left pad sequences before stacking
    max_len = max(seq.shape[1] for seq in force_end_output_sequences)
    padded_sequences = [] 
    for seq in force_end_output_sequences:
        pad_size = max_len - seq.shape[1]
        # Left pad: (left, right) for the last dimension
        padded = torch.nn.functional.pad(seq, (pad_size, 0), value=model.config.pad_token_id)
        padded_sequences.append(padded)

    
    force_end_output_sequences = torch.vstack(padded_sequences)
...
def transfer_to_device_via_cpu(tensor,target_device):
