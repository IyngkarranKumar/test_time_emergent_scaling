...
def auto_force_end_batch_scale_oom_guard(initial_size,operation_func,logger=None,description="Operation",scale_factor=0.8,min_size=1,**kwargs):
    ...
    while size >=min_size:
        try:
            ...
            if size==min_size:
                if logger is not None:
                    logger.info(f"{description} - min size reached. Breaking...")
                    raise Exception(f"{description} - min size reached. Breaking...")
                else:
                    print(f"{description} - min size reached. Breaking...")
                    raise Exception(f"{description} - min size reached. Breaking...")            
            if isinstance(e,torch.cuda.OutOfMemoryError) or is_oom_error(e):
                torch.cuda.empty_cache()
                size = max(int(size * scale_factor), min_size)
                if logger is not None:
                    logger.info(f"{description} - Force end - batch size scaled down to {size} due to CUDA OOM")
                else:
                    print(f"{description} - Force end - batch size scaled down to {size} due to CUDA OOM")
            else:
                raise e
    ...
    while size >= min_size:
        try:
            ...
            if isinstance(e,torch.cuda.OutOfMemoryError) or is_oom_error(e):
                ...
                size = max(int(size * scale_factor), min_size)
                if logger is not None:
                    logger.info(f"{description} - Scoring - batch size scaled down to {size} due to CUDA OOM")
                else:
                    print(f"{description} - Scoring - batch size scaled down to {size} due to CUDA OOM")
            else:
                raise e
