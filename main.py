...
def main():
    ...
    if os.environ.get("node_type")=="local":
        dot_env_path="env_vars/.env.local"
    elif os.environ.get("node_type")=="mlp_head":
        dot_env_path="env_vars/.env.mlp_head"
    elif os.environ.get("node_type")=="mlp_compute":
        dot_env_path="env_vars/.env.mlp_compute"
    elif os.environ.get("node_type")=="vastai":
        dot_env_path="env_vars/.env.vastai"
    elif os.environ.get("node_type")=="eddie_head":
        dot_env_path="env_vars/.env.eddie_head"
    elif os.environ.get("node_type")=="eddie_compute":
        dot_env_path="env_vars/.env.eddie_compute"
    ...
    import logging, pdb, sys,glob, subprocess, socket
    ...
    # Set pytorch attention type
    if config.ATTENTION_TYPE == "flash":
        logger.info("Enabling Flash Attention")
        torch.backends.cuda.enable_flash_sdp(True)
        torch.backends.cuda.enable_mem_efficient_sdp(True)
        torch.backends.cuda.enable_math_sdp(True)  
    elif config.ATTENTION_TYPE == "mem_efficient":
        logger.info("Enabling Mem Efficient Attention, Disabling Flash Attention")
        torch.backends.cuda.enable_flash_sdp(False)
        torch.backends.cuda.enable_mem_efficient_sdp(True)
        torch.backends.cuda.enable_math_sdp(True) 
    elif config.ATTENTION_TYPE == "vanilla":
        logger.info("Enabling Vanilla Attention, Disabling Flash and Mem Efficient Attention")
        torch.backends.cuda.enable_flash_sdp(False)
        torch.backends.cuda.enable_mem_efficient_sdp(False)
        torch.backends.cuda.enable_math_sdp(True)  # Use basic attention only
    ...
    if 1: #device mapping
        ...
        if config.inference_engine=="vllm":
            if num_gpus==1:
                ...
                vllm_devices=["cuda:0","cuda:1"]
                hf_devices=["cuda:2"]
            else: #if even, just split between the two
            ...
            vllm_devices = [] 
            hf_devices = [f"cuda:{i}" for i in range(num_gpus)]
    ...
    gen_params={
        ...
        "bad_words_ids":[[tokenizer.encode(bad_word,add_special_tokens=False)[0]] for bad_word in config.bad_words] if len(config.bad_words)>0 else None,
    ...
    #load dataset
    ds=utils.DatasetSetup(config.dataset_name,config=config)
    dataset_samples = ds.load_dataset()
    ...
    for token_iter, token_budget in enumerate(token_budgets):
        ...
        for batch_idx, sample_batch in enumerate(tqdm(sample_batches, desc=f"Processing batches (budget={token_budget})", leave=False)):
            ...
            batch_sample_idxs = [sample['sample_idx'] for sample in sample_batch]
            ...
            logger.info(f"Doing generation for batch {batch_idx + 1}/{len(sample_batches)} (sample_idxs: {batch_sample_idxs})")
            ...
            if 1: #genreation for batch
                ...
                if 1: #batch inits
                    ...
                    zero_token_diff_idxs = [] #hacky work around 
            ...
            if 1: #generation loop 
                ...
                while not batch_exit_conditions.all():
                    ...
                    logger.info(f"\n\n Starting generation loop {while_loop_idx + 1} \n\n")
                    ...
                    if 1: #Allocate samples (and consistency check) between inactive, force end, force continue, and normal gen
                        ...
                        for i in range(len(active_input_tokens)):
                            if exceeds_budget[i]:
                                ...
                                force_end_mask.append(i)
                            elif len(zero_token_diff_idxs) > 0 and zero_token_diff_idxs[i]:
                                force_end_mask.append(i) #end genreation for samples that see no token change
                        ...
                        force_end_samples = active_indices[force_end_mask].tolist()
                        force_continue_samples = active_indices[force_continue_mask].tolist()  
                        normal_gen_samples = active_indices[normal_gen_mask].tolist()
                        logger.info(f"Inactive samples: {torch.where(batch_exit_conditions)[0].tolist()}, Force continue: {force_continue_samples}, Force end: {force_end_samples}, Normal gen: {normal_gen_samples}")
                    ...
                    if 1: #FORCE END GENERATION BLOCK
                        ...
                        if len(force_end_mask) > 0:
                            ...
                            logger.info(f"Force end batch size: {config.force_end_batch_size}")
                            ...
                            with utils.gpu_util_manager("Force end generation step", logger):
                                with utils.timer("Force end generation step", logger):
                                    force_end_output_sequences,force_end_output_scores = utils.auto_force_end_batch_scale_oom_guard(
                                        initial_size = config.force_end_batch_size,
                            ...
                            for batch_local_idx, local_idx in enumerate(force_end_mask):
                                global_idx = active_indices[local_idx] #get global idx 
                                sample_idx = batch_sample_idxs[global_idx//config.num_completions] #get the corresponding sample_idx
                                completion_idx = (global_idx % config.num_completions).item()
                                ...
                                SAVE_DATA[token_budget][sample_idx]['completions'][completion_idx]['text'].append(output_text)
                                
                                #add prediction logits to save object
                                SAVE_DATA[token_budget][sample_idx]['completions'][completion_idx]['logits'] = force_end_output_scores[batch_local_idx].cpu()
                    ...
                    if 1: #FORCE CONTINUE GENERATION BLOCK
                        ...
                        if len(force_continue_mask) > 0:
                            ...
                            for local_idx in force_continue_mask:
                                ...
                                if "scotia" in socket.gethostname().lower():
                                    input_seq = utils.transfer_to_device_via_cpu(input_seq,target_device=default_force_continue_device)
                                else:
                            ...
                            if "scotia" in socket.gethostname().lower(): #scotia causes some problems - so go via cpu
                                force_continue_output_sequences = utils.transfer_to_device_via_cpu(force_continue_outputs.sequences,target_device=new_sequences.device)
                            else:   
                    ...
                    if 1:  #NORMAL GENERATION BLOCK
                            if len(normal_gen_mask) > 0:
                                ...
                                if "scotia" in socket.gethostname().lower(): #scotia causes some problems - so go via cpu
                                    normal_output_sequences = utils.transfer_to_device_via_cpu(normal_outputs.sequences,target_device=new_sequences.device)

                                else:
                                    normal_output_sequences = normal_outputs.sequences.to(new_sequences.device) #works fine elsewhere
                    ...
                    if 1: #post generation loop idx processing
                        ...
                        for local_idx, global_idx in enumerate(active_indices):
                            ...
                            if config.CHECK_FOR_FORCE_CONTINUE_REPETITIONS:
                                if global_idx in force_end_samples: #we don't want to track force continue repetition for force end samples
                                    continue
                        ...
                        if config.CHECK_FOR_FORCE_CONTINUE_REPETITIONS:
                            repetitive_seq_idxs = utils.check_for_force_continue_repetition(FORCE_CONTINUE_N_TOKEN_TRACKER,force_end_samples,n_tokens_gen_threshold=config.force_continue_repetition_threshold)


                        for local_idx, global_idx in enumerate(list(FORCE_CONTINUE_N_TOKEN_TRACKER.keys())): #global idx indexes the active indices
                            if repetitive_seq_idxs[local_idx]:
                                sample_idx_to_print, completion_idx_to_print = batch_sample_idxs[global_idx//config.num_completions], global_idx % config.num_completions
                                logger.info(f"Sequence {sample_idx_to_print}-{completion_idx_to_print} (sample_idx,completion_idx) identified as repeating with force continue tracking: {FORCE_CONTINUE_N_TOKEN_TRACKER[global_idx]}. Moving to force end")


                        #hacky way of dealing with zero_token_diffs
                        zero_token_diff_idxs = torch.zeros(len(active_indices), dtype=torch.bool, device=default_device)
                        for global_idx,token_diff_val in enumerate(token_diff):
                            if global_idx not in inactive_samples: #consider only active samples
                                if global_idx not in force_end_samples: #those in force end are not considered
                                    tmp_idx = active_indices.tolist().index(global_idx)
                                    if token_diff_val==0:
                                        sample_idx_to_print, completion_idx_to_print = batch_sample_idxs[global_idx//config.num_completions], global_idx % config.num_completions
                                        logger.info(f"Sequence {sample_idx_to_print}-{completion_idx_to_print} (sample_idx,completion_idx) identified as having no token change. Moving to force end")
                                        zero_token_diff_idxs[tmp_idx] = True
                                    else:
                                        pass
            ...
            logger.info(f"Finished generation for batch {batch_idx + 1}/{len(sample_batches)} (sample_idxs: {batch_sample_idxs})")
            ...
            if 1: #scoring for batch
                ...
                if 1: #scoring for batch
                    ...
                    if config.normalise_over_solution_set:
                        ...
                        for sample_enum_idx, sample_idx in enumerate(tqdm(batch_sample_idxs, desc=f"Processing batch samples (budget={token_budget})")):

                            if config.dataset_name=="Idavidrein/gpqa":
                                ground_truth_answer = dataset_samples[sample_enum_idx]['MCQ answer']
                            else:
                                ground_truth_answer = dataset_samples[sample_enum_idx]['answer']


                            ground_truth_tokens = tokenizer(ground_truth_answer.strip(), return_tensors='pt',add_special_tokens=False)['input_ids'][0]

                            _ground_truth_idx = np.where(solution_set==np.array(ground_truth_answer)) #indexes based on solution set distribution
                            ...
                            for completion_idx in tqdm(range(config.num_completions), desc="Processing completions)",leave=False):
                                ...
                                if config.dataset_name=="Maxwell-Jia/AIME_2024" or config.dataset_name=="math-ai/aime25":
                                    ...
                                    if 1: #calculate approximately largest max solution set batch size that fits on GPU (to avoid OOM)
                                        ...
                                        if config.calculate_solution_set_batch_size: 
                                            ...
                                            solution_set_batch_size = config.solution_set_batch_size #hardcoded. could be cleverer with this. 
                                    ...
                                    logger.info(f"Solution set batch size: {solution_set_batch_size}")
                                    ...
                                    with utils.gpu_util_manager("Solution set distribution calculation", logger,report_absolutes=True):
                                        with utils.timer("Solution set distribution calculation", logger):
                                            ...
                                            solution_set_distribution = utils.auto_scoring_batch_scale_oom_guard(initial_size=solution_set_batch_size,
                    ...
                    if not config.normalise_over_solution_set:
                        ...
                        for sample_enum_idx, sample_idx in enumerate(tqdm(batch_sample_idxs, desc=f"Processing batch samples (budget={token_budget})")):
                            ...
                            if config.dataset_name=="Idavidrein/gpqa":
                                ground_truth = dataset_samples[sample_enum_idx]['MCQ answer']
                            else:
                                ground_truth = dataset_samples[sample_enum_idx]['answer']
            ...
            if 1: #saving
                ...
                if config.SAVE_BOOL:
                    ...
                    if config.TIME_BASED_SAVING:
                        ...
                        sample_idxs_for_save = [k for k in SAVE_DATA[token_budget].keys() if k != 'metrics']

                        if time_based_save_bool: 

                            logger.info(f"Saving at time based interval")
                            
                            utils.save_data(SAVE_DATA, config.save_dir, token_budget=token_budget, sample_idxs=sample_idxs_for_save, config_save=False)
 
                            logger.info(f"Saved samples: {sample_idxs_for_save} for token budget {token_budget} to {config.save_dir} at {time.strftime('%Y-%m-%d %H:%M:%S')}")
                            ...
                            if len(batches_in_save_data)>0:
                                utils.save_data(SAVE_DATA, config.save_dir, token_budget=token_budget, sample_idxs=sample_idxs_for_save, config_save=False)
                                logger.info(f"Saved remaining samples: {sample_idxs_for_save} for token budget {token_budget} to {config.save_dir} at {time.strftime('%Y-%m-%d %H:%M:%S')}")
                                SAVE_DATA={}
                                SAVE_DATA[token_budget]={}
                                SAVE_DATA[token_budget]['metrics'] = {
                                    'answer_score': [],
                    ...
                    else: #save at end of token budget - we don't really use this much
