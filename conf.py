...
class Config:
    ...
    def _apply_cli_overrides(self):
        ...
        if hasattr(self, 'args'):
            ...
            # Handle num_samples specially (None is meaningful)
            if self.args.num_samples is not None:
    ...
    def _set_base_config(self):
        ...
        self.num_samples = None
        self.batch_size = 4
        self.solution_set_batch_size = 20
        ...
        self.sample_idxs = None #for selecting a subset of samples to run
        self.USE_LOCAL_HF_CACHE = None
        self.force_end_batch_size = None

        #force continue config
        self.force_continue = " Hmm, but let me keep thinking..."
        ...
        self.APPEND_EOS_TOKEN = False #This should usually be false, just for debugging. 
        ...
        self.solution_set_batch_size = 20 #number of candidate solutions to compute p over in parallel
        self.calculate_solution_set_batch_size = False
        # localisation config 
        # we can also add more granularity to localisation stage (e.g: specifying list of floating points in [0,1] to pinpoint where we want to study)
    ...
    def _set_derived_attributes(self):
        ...
        if self.LOCALISATION_RUN:
            ...
            assert len(self.start_token_budget) == len(self.sample_idxs), "start_token_budget and sample_idxs must have the same length"
        ...
        if self.max_model_len is None:
            ...
            max_model_len_buffer = max(1024,int(0.25*self.max_model_len))
        ...
        self.save_dir = self.data_dir + f"/{self.run_name}"
        if os.environ.get("USER") == "s2517451": 
            self.save_dir = remote_disk_dir + "/" + self.save_dir
        if os.environ.get("USER") == "root": #vast a 
            self.save_dir = "./results_data/"+f"{self.run_name}"
        if self.LOCALISATION_RUN:
        ...
        if self.LOCALISATION_RUN:
            ...
            assert self.sample_idxs is not None, "Localisation run requires sample_idxs to be specified"
    ...
    def __str__(self):
        ...
        sections = {
            ...
            "Token Budget": [
                ...
                ("Token Batch Mapping", self.token_batch_mapping)
            ],
            ...
            "Localisation Settings": [
                ...
                ("Localisation Sample Indices", self.sample_idxs if self.LOCALISATION_RUN else 'N/A')
            ],
